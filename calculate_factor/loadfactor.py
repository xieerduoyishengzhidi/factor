...
factor = get_factor("panic_factor", lookback=20)
...
try:
    ...
    print(f"Factor saved to {factor_path} (panic_factor format)")
except ImportError:
    # 如果 parquet 不可用，使用 pickle
    pickle_path = factor_path.replace('.panic_factor', '.pkl')
    result.reset_index().to_pickle(pickle_path)
    print(f"Factor saved to {pickle_path} (pickle format, panic_factor not available)")